...
def get_response(user_input: str):
    top_k_results = search_top_k(query=user_input, top_k=3)
    indices = [result["id"] for result in top_k_results]
    top_chunks = get_chunks_by_index(indices, chunks)
    context = "\n---\n".join(top_chunks)

    system_prompt = """You are a helpful assistant for Crustdata.

**What is Crustdata?**
Crustdata provides programmatic access to firmographic and growth metrics data for companies worldwide, aggregated from over 16 datasets. These datasets include platforms such as LinkedIn, Glassdoor, Instagram, G2, web traffic sources, Apple App Store reviews, Google Play Store, and news articles.

**Your Task:**
Respond promptly and informatively to user inquiries concerning the Crustdata API.

**Answer Guidelines:**
- **Clarity and Conciseness:** Provide responses that are clear and to the point.
- **Use HTML Tags for Formatting:**
  - Utilize tags such as `<ul>`, `<strong>`, and `<p>` to improve the readability of the content.
- **Relevance:** Keep your responses brief. Elaborate only if the additional details are directly relevant and add significant value.
- **Focus:** Do not address questions that are unrelated to the Crustdata API.

**Note:** Ensure to maintain a professional and helpful tone throughout your interactions.
...
st.set_page_config(page_title="Crustdata Chatbot", layout="centered")

# Set title for the app
st.title("Crustdata API Support Chatbot")
...
def display_chat():
    for msg in st.session_state.messages:
        if msg.get("user"):
            st.markdown(f"**You** 🙋: <div>{msg['user']}</div>", unsafe_allow_html=True)
        if msg.get("bot"):
            st.markdown(f"**Bot** 🤖: <div>{msg['bot']}</div>", unsafe_allow_html=True)

# Display existing chat history
display_chat()

# Spacer for the chat input to be at the bottom
st.markdown("<br>", unsafe_allow_html=True)

# User chat interface: Text input box and Send button at the bottom
user_input = st.text_input("Ask a question about Crustdata's APIs:", key="input", placeholder="Type your question here...")
...
#         response = {
#             "status_code": 200,
#             "answer": """To search for people based on their current title, current company, and location using the Crustdata API, you can utilize the `filters` key in your request body. Below is an example of how to format your request:
...
# ### Key Components:
# - **filter_type**: Use `CURRENT_TITLE`, `CURRENT_COMPANY`, and `LOCATION` to specify the criteria for filtering.
# - **type**: Typically, you will use `equals` for exact matches.
# - **value**: Replace these placeholders with the specific title, company, and location you want to search for.
# - **page**: Start with `1` for the first page of results; you can increment this to paginate through additional results.

# This request will return people who match all the specified criteria. Adjust the `value` fields as necessary to suit your search requirements."""
#         }
        response = get_response(user_input)
        st.session_state.messages.append({"user": user_input, "bot": response})
    st.rerun()

# Make sure the input box stays at the bottom of the page
st.markdown("<br>", unsafe_allow_html=True)